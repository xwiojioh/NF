...
type (
	...
	Address [addressLength]byte // Address is the only kind of way used to represent users, nodes, contracts and contract value storage in p3Chain.
...
func BytesToWsKey(ins []byte) WsKey {
	wsKey := WsKey{}
	copy(wsKey[:], ins)
	return wsKey
}
