...
func startTestCluster(n int) []*Whisper {
	...
	nodes := make([]*server.Peer, n)
	for i := 0; i < n; i++ {
		nodes[i] = server.NewPeer(discover.NodeID{}, "", nil)
	}
	...
	for i := 1; i < n; i++ {
		src, dst := server.MsgPipe()

		go whispers[0].handlePeer(nodes[i], src, ctx)
		go whispers[i].handlePeer(nodes[0], dst, ctx)
	}
	return whispers
}
