...
var discard = Protocol{
	Name:   "discard",
	Length: 1,
	Run: func(p *Peer, rw MsgReadWriter, ctx context.Context) error {
		for {
			msg, err := rw.ReadMsg()
			if err != nil {
				return err
...
func TestPeerProtoReadMsg(t *testing.T) {
	...
	proto := Protocol{
		Name:   "a",
		Length: 5,
		Run: func(peer *Peer, rw MsgReadWriter, ctx context.Context) error {
...
func TestPeerDisconnectRace(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		closer, rw, p, disc := testPeer([]Protocol{
			{
				Name:   "closereq",
				Run:    func(p *Peer, rw MsgReadWriter, ctx context.Context) error { return <-protoclose },
				Length: 1,
			},
			{
				Name:   "disconnect",
				Run:    func(p *Peer, rw MsgReadWriter, ctx context.Context) error { p.Disconnect(<-protodisc); return nil },
				Length: 1,
			},
		})
