...
//
// # This is maily realized by using the sync.Once
//
...
func (db *nodeDB) querySeeds(n int) []*Node {
	// Use a local iterator instead of shared one to avoid concurrency issues
	// This prevents nil pointer dereference when multiple goroutines access the iterator
	it := db.lvl.NewIterator(nil, nil)
	defer it.Release()
	...
	for len(nodes) < n && it.Next() {
		...
		// Dump it if it's a self reference
		if bytes.Compare(id[:], db.self[:]) == 0 {
			db.deleteNode(id)
			continue
		}
