...
func TestTable_pingReplace(t *testing.T) {
	doit := func(newNodeIsResponding, lastInBucketIsResponding bool, idx int) {
		...
		if !transport.pinged[pingSender.ID] {
			t.Errorf("error in test %d: table did not ping back sender", idx)
		}
		if newNodeIsResponding {
			// second ping goes to oldest node in bucket
			// to see whether it is still alive.
			if !transport.pinged[last.ID] {
				t.Errorf("error in test %d: table did not ping last node in bucket", idx)
			}
		}
		...
		if l := len(tab.buckets[253].entries); l != bucketSize {
			t.Errorf("error in test %d: wrong bucket size after bond: got %d, want %d", idx, l, bucketSize)
		}

		if lastInBucketIsResponding || !newNodeIsResponding {
			if !contains(tab.buckets[253].entries, last.ID) {
				t.Errorf("error in test %d: last entry was removed", idx)
			}
			if contains(tab.buckets[253].entries, pingSender.ID) {
				t.Errorf("error in test %d: new entry was added", idx)
			}
		} else {
			if contains(tab.buckets[253].entries, last.ID) {
				t.Errorf("error in test %d: last entry was not removed", idx)
			}
			if !contains(tab.buckets[253].entries, pingSender.ID) {
				t.Errorf("error in test %d: new entry was not added", idx)
			}
		}
	}
