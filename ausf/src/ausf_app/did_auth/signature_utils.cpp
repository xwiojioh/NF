...
std::string signature_utils::build_signature_payload(
  ...
  // 简化payload：直接使用challenge的二进制数据
  // 这样可以确保签名和验签使用完全相同的数据
  // 如果需要包含时间戳，可以扩展payload结构
  
  // 目前保持简单：payload就是challenge本身（二进制形式）
...
std::string signature_utils::normalize_public_key(const std::string& public_key_hex) {
  ...
  if (key_bytes.empty()) {
    Logger::ausf_app().error("[SigUtils] Empty public key");
    return "";
  }
  ...
  if (key_bytes.size() == 65) {
    ...
    with_prefix.insert(with_prefix.end(), key_bytes.begin(), key_bytes.end() - 1);
  ...
  if (key_bytes.size() == 33) {
    ...
    if (key_bytes[0] == 0x02 || key_bytes[0] == 0x03) {
      ...
      if (uncompressed.empty()) {
        Logger::ausf_app().error("[SigUtils] Failed to decompress public key");
        return "";
      }
      return to_hex(uncompressed);
    }
  }
...
std::string signature_utils::ecdsa_sign(
  ...
  EC_KEY* ec_key = EC_KEY_new_by_curve_name(NID_secp256k1);
  if (!ec_key) {
    Logger::ausf_app().error("[SigUtils] Failed to create EC_KEY");
    return "";
  }
  ...
  if (!priv_bn) {
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] Failed to create BIGNUM for private key");
    return "";
  }
  ...
  if (EC_KEY_set_private_key(ec_key, priv_bn) != 1) {
    BN_free(priv_bn);
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] Failed to set private key");
    return "";
  }
  ...
  if (!sig) {
    BN_free(priv_bn);
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] ECDSA_do_sign failed");
    return "";
  }
  ...
  if (der_len <= 0) {
    ...
    BN_free(priv_bn);
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] Failed to encode signature to DER");
    return "";
  }
...
bool signature_utils::ecdsa_verify(
  ...
  if (hash.size() != 32) {
    Logger::ausf_app().error("[SigUtils] Invalid hash size: %zu", hash.size());
    return false;
  }
  ...
  if (!pub_point) {
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] Failed to create EC_POINT");
    return false;
  }
  ...
  if (EC_POINT_oct2point(group, pub_point, public_key.data(), 
    ...
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] Failed to decode public key point");
    return false;
  }
  ...
  if (EC_KEY_set_public_key(ec_key, pub_point) != 1) {
    EC_POINT_free(pub_point);
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] Failed to set public key");
    return false;
  }
  ...
  if (!sig) {
    EC_POINT_free(pub_point);
    EC_KEY_free(ec_key);
    Logger::ausf_app().error("[SigUtils] Failed to decode DER signature");
    return false;
  }
...
std::string signature_utils::sign_challenge_with_logging(
  ...
  if (signature.empty()) {
    Logger::ausf_app().error("[SigUtils] sign_challenge FAILED");
    return "";
  }
  
  // 只输出关键结果（调试时可开启 trace 级别看完整参数）
  Logger::ausf_app().debug("[SigUtils] sign_challenge SUCCESS (sig_len=%zu)", signature.length());
...
bool signature_utils::verify_challenge_with_logging(
    ...
    uint64_t timestamp_ms) {
  // Step 1: Normalize public key (silently)
  std::string normalized_pk = normalize_public_key(public_key_hex);
  if (normalized_pk.empty()) {
    Logger::ausf_app().error("[SigUtils] Failed to normalize public key");
    return false;
  }
  ...
  // 只输出关键结果
  if (result) {
    Logger::ausf_app().info("[SigUtils] verify_challenge SUCCESS");
  } else {
    Logger::ausf_app().error("[SigUtils] verify_challenge FAILED");
  }
