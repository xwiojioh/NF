...
//------------------------------------------------------------------------------
void ausf_profile::set_nf_plmn_list(const std::vector<plmn_item_t>& p) {
  plmn_list = p;
}
...
//------------------------------------------------------------------------------
void ausf_profile::get_nf_plmn_list(std::vector<plmn_item_t>& p) const {
  p = plmn_list;
}
...
void ausf_profile::display() const {
  ...
  if (!plmn_list.empty()) {
    Logger::ausf_app().debug("    PLMN Support:");
  }
  for (const auto& plmn : plmn_list) {
    Logger::ausf_app().debug(
        "        MCC, MNC: %s, %s", plmn.mcc.c_str(), plmn.mnc.c_str());
    Logger::ausf_app().debug("        TAC: %u", plmn.tac);
  }
  // SNSSAIs
  if (snssais.size() > 0) {
...
void ausf_profile::to_json(nlohmann::json& data) const {
  ...
  if (!plmn_list.empty()) {
    network_support::append_nf_profile_network_support(data, plmn_list);
  } else {
    data["sNssais"] = nlohmann::json::array();
    for (auto s : snssais) {
      data["sNssais"].push_back(network_support::snssai_to_json(s));
    }
  }
  data["fqdn"] = fqdn;
  // ipv4_addresses
...
void ausf_profile::from_json(const nlohmann::json& data) {
  ...
  if (data.find("plmnList") != data.end()) {
    plmn_list.clear();
    for (const auto& it : data["plmnList"]) {
      plmn_item_t item = {};
      item.mcc         = it.value("mcc", "");
      item.mnc         = it.value("mnc", "");
      plmn_list.push_back(item);
    }
  }
  if (data.find("perPlmnSnssaiList") != data.end()) {
    for (const auto& it : data["perPlmnSnssaiList"]) {
      if (!it.contains("plmnId")) {
        continue;
      }

      const auto& plmn_id = it["plmnId"];
      const std::string mcc = plmn_id.value("mcc", "");
      const std::string mnc = plmn_id.value("mnc", "");

      auto existing = std::find_if(
          plmn_list.begin(), plmn_list.end(),
          [&mcc, &mnc](const plmn_item_t& item) {
            return item.mcc == mcc && item.mnc == mnc;
          });
      if (existing == plmn_list.end()) {
        plmn_item_t item = {};
        item.mcc         = mcc;
        item.mnc         = mnc;
        plmn_list.push_back(item);
        existing = std::prev(plmn_list.end());
      }

      existing->slice_list.clear();
      if (it.contains("sNssaiList")) {
        for (const auto& slice_it : it["sNssaiList"]) {
          snssai_t slice = {};
          slice.sst      = slice_it.value("sst", 0);
          if (slice_it.contains("sd")) {
            slice.sd = slice_it["sd"].get<std::string>();
          }
          existing->slice_list.push_back(slice);
        }
      }
    }
  }
