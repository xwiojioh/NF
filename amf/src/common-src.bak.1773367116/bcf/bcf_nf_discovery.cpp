...
NfProfile NfProfile::from_json(const nlohmann::json& j) {
  ...
  if (j.contains("nfInstanceId"))
    p.nf_instance_id = j["nfInstanceId"].get<std::string>();
  if (j.contains("nfType"))
    p.nf_type = string_to_nf_type(j["nfType"].get<std::string>());
  if (j.contains("nfStatus"))
    p.nf_status = string_to_nf_status(j["nfStatus"].get<std::string>());

  if (j.contains("did")) p.did = j["did"].get<std::string>();
  if (j.contains("publicKey")) p.public_key = j["publicKey"].get<std::string>();

  if (j.contains("plmnList")) {
    for (const auto& plmn : j["plmnList"]) {
      p.plmn_list.push_back(PlmnId::from_json(plmn));
    }
  }

  if (j.contains("sNssais")) {
    for (const auto& snssai : j["sNssais"]) {
      p.snssai_list.push_back(Snssai::from_json(snssai));
    }
  }

  if (j.contains("nsiList"))
    p.nsi_list = j["nsiList"].get<std::vector<std::string>>();
  if (j.contains("fqdn")) p.fqdn = j["fqdn"].get<std::string>();
  if (j.contains("ipv4Addresses"))
    p.ipv4_addresses = j["ipv4Addresses"].get<std::vector<std::string>>();
  if (j.contains("ipv6Addresses"))
    p.ipv6_addresses = j["ipv6Addresses"].get<std::vector<std::string>>();

  if (j.contains("nfServices")) {
    for (const auto& svc : j["nfServices"]) {
      p.nf_services.push_back(NfService::from_json(svc));
    }
  }

  if (j.contains("priority")) p.priority = j["priority"].get<int32_t>();
  if (j.contains("capacity")) p.capacity = j["capacity"].get<int32_t>();
  if (j.contains("load")) p.load = j["load"].get<int32_t>();
  if (j.contains("locality")) p.locality = j["locality"].get<std::string>();
...
PublicKeyResponse BcfNfDiscoveryClient::query_public_key(const std::string& did) {
  ...
  // 构建查询路径
  std::string path = "/nbcf-nfm/" + m_bcf_api_version + "/did/" + did + "/publickey";
...
std::string BcfNfDiscoveryClient::build_discovery_uri(
  ...
  std::string path =
      "/nbcf_nfm/" + m_bcf_api_version + "/nf_instances?" + criteria.to_query_params();
  return path;
}
