...
bool LcsClientGroupExternal::validate(
    std::stringstream& msg, const std::string& pathPrefix) const {
  bool success = true;
  const std::string _pathPrefix =
      pathPrefix.empty() ? "LcsClientGroupExternal" : pathPrefix;
...
bool LcsClientGroupExternal::operator==(
    const LcsClientGroupExternal& rhs) const {
  return

      ((!lcsClientGroupIdIsSet() && !rhs.lcsClientGroupIdIsSet()) ||
       (lcsClientGroupIdIsSet() && rhs.lcsClientGroupIdIsSet() &&
        getLcsClientGroupId() == rhs.getLcsClientGroupId())) &&
...
LcsClientGroupExternal::getAllowedGeographicArea() const {
...
oai::_3gpp::model::ValidTimePeriod LcsClientGroupExternal::getValidTimePeriod()
    const {
  return m_ValidTimePeriod;
}
void LcsClientGroupExternal::setValidTimePeriod(
