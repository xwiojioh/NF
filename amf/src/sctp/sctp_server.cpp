...
void sctp_server::start_receive(sctp_application* app) {
  ...
  if (receiver_thread_started_) {
    Logger::sctp().warn("sctp_receiver_thread is already running");
    return;
  }
  ...
  if (res != 0) {
    Logger::sctp().error(
        "Failed to create sctp_receiver_thread: %s", strerror(res));
    return;
  }
...
//------------------------------------------------------------------------------
void sctp_server::stop_receive() {
  stop_requested_.store(true);

  if (!socket_closed_ && socket_ >= 0) {
    if (close(socket_) != 0) {
  ...
  if (receiver_thread_started_) {
    const int res = pthread_join(thread_, NULL);
    if (res != 0) {
      Logger::sctp().error(
          "pthread_join on sctp_receiver_thread failed %s", strerror(res));
    } else {
      Logger::sctp().debug("sctp_receiver_thread joined successfully");
    }
    receiver_thread_started_ = false;
  }

  Logger::sctp().debug("Thread on sctp_receiver_thread has fully ended");
}
...
void* sctp_server::sctp_receiver_thread(void* arg) {
  ...
  while (!ptr->stop_requested_.load()) {
    memcpy(&read_fds, &master, sizeof(master));
    struct timeval timeout = {.tv_sec = 0, .tv_usec = 200000};
    const int select_rc =
        select(fdmax + 1, &read_fds, NULL, NULL, &timeout);

    if (ptr->stop_requested_.load()) {
      break;
    }
  ...
  for (int i = 0; i <= fdmax; i++) {
    if (FD_ISSET(i, &master) && (i != ptr->get_socket())) {
      if (close(i) != 0) {
        Logger::sctp().warn(
            "close on accepted socket(%d) failed %s", i, strerror(errno));
      }
    }
  }

  Logger::sctp().debug("sctp_receiver_thread exited");
  return NULL;
}
...
//------------------------------------------------------------------------------
void sctp_server::cleanup_associations() {
  for (auto* association : sctp_ctx_) {
    if (!association) continue;

    if (association->peer_addresses) {
      sctp_freepaddrs((struct sockaddr*) association->peer_addresses);
      association->peer_addresses = nullptr;
    }
