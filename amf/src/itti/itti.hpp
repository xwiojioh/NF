...
class itti_mw {
 ...
 public:
  ...
  /** 
   * @brief Mark a given task as ended (thread is exiting). This allows external
   * tasks to notify ITTI about their exit without accessing internal members.
   */
  void mark_task_ended(const task_id_t task_id);

  /** 
   * @brief Query whether ITTI has been asked to terminate.
   * This provides a safe, public accessor to the internal terminate flag so
   * other modules can adapt behavior during shutdown without accessing
