...
//------------------------------------------------------------------------------
bool itti_mw::is_terminating() const { return terminate; }
//------------------------------------------------------------------------------
itti_mw::itti_mw()
    : timer_id(0),
      msg_number(0),
...
itti_mw::~itti_mw() {
  ...
  // Always join the timer thread if it was started. Otherwise the underlying
  // std::thread would still be joinable here and its destructor would trigger
  // std::terminate() during AMF shutdown.
...
int itti_mw::send_terminate_msg(task_id_t src_task_id) {
  // Build terminate message targeted to all tasks
  itti_msg_terminate msg(src_task_id, TASK_ALL);

  // Set global terminate flag so threads that check it can stop
  terminate = true;

  std::shared_ptr<itti_msg_terminate> smsg =
      std::make_shared<itti_msg_terminate>(msg);

  // Wake up timer thread(s) which may be waiting on c_timers / c_timeout
  c_timers.notify_all();
  c_timeout.notify_all();

  // Push terminate message into every task queue and notify them so that
  // blocked receive_msg/c_queue.wait can return immediately. This ensures
  // tasks that are not in TASK_STATE_READY still get the terminate message
  // and can exit their loops.
  for (int t = TASK_FIRST; t < TASK_MAX; t++) {
    if (itti_task_ctxts[t]) {
      {
        std::unique_lock<std::mutex> l(itti_task_ctxts[t]->m_queue);
        itti_task_ctxts[t]->msg_queue.push(smsg);
      }
      itti_task_ctxts[t]->c_queue.notify_one();
    }
  }
...
//------------------------------------------------------------------------------
void itti_mw::mark_task_ended(const task_id_t task_id) {
  if ((TASK_FIRST <= task_id) && (TASK_MAX > task_id)) {
    if (itti_task_ctxts[task_id]) {
      std::unique_lock<std::mutex> lk(itti_task_ctxts[task_id]->m_state);
      itti_task_ctxts[task_id]->task_state = TASK_STATE_ENDED;
