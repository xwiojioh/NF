...
bool AccessTokenRsp::validate(
    std::stringstream& msg, const std::string& pathPrefix) const {
  bool success = true;
  const std::string _pathPrefix =
      pathPrefix.empty() ? "AccessTokenRsp" : pathPrefix;
...
bool AccessTokenRsp::operator==(const AccessTokenRsp& rhs) const {
  return
      ...
      ((!expiresInIsSet() && !rhs.expiresInIsSet()) ||
       (expiresInIsSet() && rhs.expiresInIsSet() &&
        getExpiresIn() == rhs.getExpiresIn())) &&

      ((!scopeIsSet() && !rhs.scopeIsSet()) ||
       (scopeIsSet() && rhs.scopeIsSet() && getScope() == rhs.getScope()))
